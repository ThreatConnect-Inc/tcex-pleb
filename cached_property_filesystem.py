...
_CACHE_DIR = Path(tempfile.gettempdir()) / 'tcex_cache'
...
class cached_property_filesystem(Generic[R]):  # noqa: N801
    ...
    def __init__(self, ttl: int = 86400):
        ...
        Args:
            ttl: Seconds before the cached value expires in both memory and on disk.
        """
        self.ttl = ttl
    ...
    def __get__(self, instance: Any, owner: type | None = None) -> R:
        ...
        now = time.time()
        cache_key = f'__cache_fs_{self.attr_name}'
        ...
        cache_path = self._cache_path(instance)
        value = self._read_fs_cache(cache_path, now)
        ...
        instance.__dict__[cache_key] = (now, value)
        self.instances.add(instance)
        self._write_fs_cache(cache_path, value, now)
    ...
    def _cache_path(self, instance: Any) -> Path:
        ...
        """
        return _CACHE_DIR / f'{type(instance).__name__}_{self.attr_name}.pkl'

    def _read_fs_cache(self, path: Path, now: float) -> R | None:
        ...
        try:
            ...
            if now - raw['timestamp'] < self.ttl:
                return raw['value']
        except (pickle.UnpicklingError, KeyError, OSError, EOFError) as ex:
